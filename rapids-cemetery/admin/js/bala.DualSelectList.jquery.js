(function($) {
	...
	$.fn.DualSelectList = function(parameter)
	{
		...
		// ALTERED BY: Team Hong Kong
		this.setCandidate = function (candidate) {
			for (var n=0; n<candidate.length; ++n) {
				var nameStr = $.trim(candidate[n]["name"].toString());
				var id = candidate[n]["id"];
				if (nameStr == '') continue;
				thisLftPanel.append('<div class="dsl-panel-item" data-poiID = ' + id + '>' + nameStr + '</div>');
			}
		};
		...
		this.setSelection = function (selection) {
			for (var n=0; n<selection.length; ++n) {
				var nameStr = $.trim(selection[n]["name"].toString());
				var id = selection[n]["id"];
				if (nameStr == '') continue;
				thisRgtPanel.append('<div class="dsl-panel-item" data-poiID = ' + id + '>' + nameStr + '</div>');
			}
		};
		...
		this.getSelection = function () {
			...
			for (var n=0; n<selection.length; ++n) result.push({"poiID": parseInt(selection.eq(n).attr("data-poiID")), "tour_order": n + 1});
			return result;
		};
